---
created: 2025-01-06
modified: 2025-01-06
---
Renown was an app for friend groups to collectively curate content. My own friends used it to keep pictures from our favorite moments together along with a list of restaurants and cafes we wanted to try. Renown made it easy to take any content (photos, website) and turn it into an beautiful list.

Some things I built working at Renown include:
- Flutter cross-platform mobile app (both designed and developed it)
- Node.js and GraphQL backend
- Web scraping to get images from URLs
- Image classification ML model for classifying which images were aesthetically pleasing
- LLM integration for providing recommendations based on current list items

![[renown-2.png|300]]
![[renown-1.png|300]]
![[renown-3.png|300]]